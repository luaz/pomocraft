...
function updateTimer() {
  ...
  if (seconds > 0) {
    if (state.mode == Mode.RUNNING || state.mode == Mode.RESTING) {
      const realSeconds =
        (state.mode == Mode.RUNNING ? POMO_FOCUS_SECONDS : POMO_REST_SECONDS) -
        Math.ceil((new Date() - timerStartTime) / 1000);
      if (Math.abs(realSeconds - seconds) >= 2) {
        console.log(`Adjust ${seconds} => ${realSeconds}`);
        seconds = realSeconds;
        if (seconds < 0) {
          // to avoid funny ui issues
          seconds = 0;
        }
      }
    }
  ...
  if (seconds >= 0) {
    ...
    if (seconds == -1) {
      ...
      showNotification(
        "Pomodoro Completed",
        [{ action: "takeBreak", title: "Take Break" }],
        (event) => {
...
watch(
  ...
  (timerText) => {
    ...
    if (state.mode == Mode.RESTING) document.title = `[B] ${timerText}`;
    else document.title = timerText;
  },
);
...
<template>
  <div
    v-bind="$attrs"
    class="flex w-48 py-4 px-4 rounded-md"
    :class="{
      'bg-slate-100': state.mode == Mode.IDLE,
      'bg-lime-100': state.mode == Mode.RUNNING,
      'bg-red-100': state.mode == Mode.COMPLETED,
      'bg-orange-100': state.mode == Mode.RESTING,
    }"
  >
    <div class="flex-1 items-center justify-center">
      <div
        v-if="state.mode == Mode.RUNNING"
        class="text-center text-sm text-slate-500 cursor-pointer"
        @click="emit('startFocus')"
      >
        {{ modeText }}
      </div>
      ...
      <div
        class="text-center text-5xl font-medium dark:text-slate-500 w-32"
        :class="{ 'animate-blink': state.mode == Mode.COMPLETED }"
      >
        {{ state.timerText }}
      </div>
      <div class="flex justify-center">
        <div
          class="text-center text-sm text-slate-700 font-medium w-32 whitespace-nowrap overflow-hidden text-ellipsis"
        >
    ...
    <div class="flex justify-center items-center">
      <UButton variant="link" tabindex="-1">
        <UIcon
          v-show="state.mode == Mode.IDLE || state.mode == Mode.RESTING"
          name="i-heroicons-play-circle"
          class="size-8"
          @click="changeMode"
        />
        <UIcon
          v-show="state.mode == Mode.RUNNING"
          name="i-heroicons-stop-circle"
          class="size-8"
          @click="changeMode"
        />
        <UIcon
          v-show="state.mode == Mode.COMPLETED"
